...
tlds.each do |tld|
  possibilities = []
  lines.each do |line|
    if line[-(tld.size)..-1].eql?(tld) and line.size > tld.size + 2
      possibilities << "http://" + line[0..-(tld.size + 1)] + "." + line[-(tld.size)..-1] + "/"
      counter += 1
    end
  end
  
  if possibilities.size > 0
    puts "--> " + possibilities.size.to_s + " matches found for ." + tld
    possibilities.each{|p| puts p}
  end
end

puts counter.to_s + " total matches found out of " + lines.size.to_s + " words."